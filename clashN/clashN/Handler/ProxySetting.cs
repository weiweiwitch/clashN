...
internal class ProxySetting
{
    ...
    private static bool SetProxy(string strProxy, string exceptions, int type)
    {
        ...
        var mInt = (int)PerConnFlags.PROXY_TYPE_DIRECT;
        var mOption = PerConnOption.INTERNET_PER_CONN_FLAGS;
        if (type == 2)
        {
            mInt = (int)(PerConnFlags.PROXY_TYPE_DIRECT | PerConnFlags.PROXY_TYPE_PROXY);
            mOption = PerConnOption.INTERNET_PER_CONN_PROXY_SERVER;
        }
        else if (type == 4)
        {
            mInt = (int)(PerConnFlags.PROXY_TYPE_DIRECT | PerConnFlags.PROXY_TYPE_AUTO_PROXY_URL);
            mOption = PerConnOption.INTERNET_PER_CONN_AUTOCONFIG_URL;
        }
        ...
        options[0].m_Value.m_Int = mInt;
        // use THIS proxy server
        if (optionCount > 1)
        {
            options[1].m_Option = mOption;
